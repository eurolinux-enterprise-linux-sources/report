...
class Bugzilla3(base.BugzillaBase):
    '''Concrete implementation of the Bugzilla protocol. This one uses the
    methods provided by standard Bugzilla 3.0.x releases.'''

    version = '0.1'
    user_agent = base.user_agent + ' Bugzilla3/%s' % version
...
class Bugzilla32(Bugzilla3):
    ...
    version = '0.1'
    user_agent = base.user_agent + ' Bugzilla32/%s' % version
    createbug_required = ('product','component','summary','version')
...
class Bugzilla34(Bugzilla32):
    version = '0.1'
    user_agent = base.user_agent + ' Bugzilla34/%s' % version
    ...
    def _getusers(self, ids=None, names=None, match=None):
        ...
        if match:
            params['match'] = match
        if not params:
            raise base.NeedParamError('_get() needs one of ids,'
