...
def report(signature, io, optionsDict):
    ...
    if optionsDict.has_key("testing_component"):
        component = optionsDict["testing_component"]
    elif 'component' in signature:
        component = signature["component"].asString()
    else:
        component = None

    if 'hashmarkername' in signature:
        hashmarkername = signature["hashmarkername"].asString()
    else:
        hashmarkername = None

    if 'localhash' in signature:
        localhash = signature["localhash"].asString()
    else:
        localhash = None
...
# 
def sendToBugzilla( component, hashmarkername, localhash, summary, description, fileName, fileDescription, io, optionsDict, bzfiler):
    ...
    try:
        ...
        try:
            withBugzillaDo(bzfiler, lambda b: b.login(username, password))
        ...
        if 'ticket' in optionsDict:
        ...
        elif localhash and hashmarkername:
            ...
            wb = "%s_trace_hash:%s" % (hashmarkername, localhash)
            buglist = withBugzillaDo(bzfiler, lambda b: b.query(
                    {'status_whiteboard': wb,
                     'status_whiteboard_type':'allwordssubstr',
                     'bug_status': []}))

        elif component and (fileDescription or description):
