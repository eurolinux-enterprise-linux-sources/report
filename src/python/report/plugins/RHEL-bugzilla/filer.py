...
_hardcoded_default_product = 'Red Hat Enterprise Linux'
_hardcoded_default_version = '6.0'
_hardcoded_default_product_for_bugzilla = 'Red Hat Enterprise Linux 6'
...
class BugzillaFiler(AbstractFiler):
    ...
    def login(self, username, password):
        ...
        try:
            self._bz = bugzillaCopy.Bugzilla(url=self.bugUrl)
    ...
    def getproduct(self):
        ...
        # Extend product with high order number of version
        product_with_version = self.product + ' ' + self.version.split('.')[0]
        for d in details:
            if d['name'] == product_with_version:
                return product_with_version
        ...
        product_with_version = getProduct() + ' ' + getVersion().split('.')[0]
        for d in details:
            if d['name'] == product_with_version:
                return product_with_version

        return _hardcoded_default_product_for_bugzilla
    ...
    def getversion(self):
        ...
        # (5.4, 5.5, 6.0).
        details = self.__withBugzillaDo(lambda b: b._proxy.bugzilla.getProductDetails(self.getproduct()))
...
class BugzillaBug(AbstractBug):
    ...
    def __init__(self, filer, bug=None, *args, **kwargs):
        ...
        if not bug:
            self._bug = bugzillaCopy.Bug(self.filer, *args, **kwargs)
        else:
            self._bug = bug
